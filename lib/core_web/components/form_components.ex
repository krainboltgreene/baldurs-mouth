defmodule CoreWeb.FormComponents do
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select id={@id} name={@name} class="mt-2 block w-full rounded-md border border-gray-300 bg-white shadow-sm focus:border-zinc-400 focus:ring-0" multiple={@multiple} {@rest}>
  ...
  def error(assigns) do
    ~H"""
    <p id={if(@describing, do: "#{@describing}_feedback")} class="mt-3 flex gap-3 text-sm leading-6 text-rose-600 phx-no-feedback:hidden">
      <%= render_slot(@inner_block) %>
    </p>
    """
  end
